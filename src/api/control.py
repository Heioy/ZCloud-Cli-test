...
class ZStackControl(Base):
    ...
    def show_ui_config(self):
        """List All UI Config"""
        command = f"{self.zstack_ctl} {commands.ZStack_Show_UI_Config}"
    ...
    def version(self):
        """Get ZStack Version"""
        command = f"{self.zstack_ctl} {commands.ZStack_Get_Version}"
        response = self.client.run_command(command)
        return response['stdout']

    def show_sessions(self):
        """Show Current Login Session"""
        command = f"{self.zstack_ctl} {commands.ZStack_show_Session_list}"
        response = self.client.run_command(command)
    ...
    def drop_account_session(self, account: str = None):
        ...
        if not account:
            print("清除所有session...")
            command = f"{self.zstack_ctl} {commands.ZStack_Drop_Session} --all"
        elif account:
            command = f"{self.zstack_ctl} {commands.ZStack_Drop_Session} --account {account}"
        response = self.client.run_command(command)
